...
export function DaySelector({ selected, selectDate }: { selected: Day, selectDate: (date: Day) => void }) {
    ...
    const currDay: Day = useMemo(() => { return { date: selected.date, month: selected.month, year: selected.year, weekday: selected.weekday } }, [selected])
            ...
            {getDaysInMonth(month.month, month.year).map((day: Day | undefined, i: number) => {
                if (day !== undefined) {
                    return (<div key={"day-" + day.date + "-" + day.month + "-" + day.year} className={`transition-all duration-300 ease-in-out col-start-${i % 7 + 1} row-start-${Math.floor(i / 7) + 1}`}>
                        <button onClick={() => { selectDate(day); }} className={`transition-all duration-300 ease-in-out w-8 h-8 text-center rounded-3xl ${daysAreEqual(currDay, day) ? "bg-blue-500 hover:bg-blue-400 text-white" : "hover:bg-opacity-30 hover:bg-blue-500"}`}>
                            {day.date}
                        </button>
                    </div>);
                }
